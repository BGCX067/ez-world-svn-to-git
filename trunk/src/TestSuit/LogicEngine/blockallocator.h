 ...
 * LEGAL:
 *
 * (C) 2000-2006 Joaqu韓 M?L髉ez Mu駉z (joaquin@tid.es). All rights reserved.
 *
 ...
 *       "Portions copyright 2000-2006 Joaqu韓 M?L髉ez Mu駉z (joaquin@tid.es)"
 *
...
#ifdef DEFINE_BLOCK_ALLOCATED_HASH_SET
#ifndef BLOCK_ALLOCATED_HASH_SET_DEFINED
#define BLOCK_ALLOCATED_HASH_SET_DEFINED
...
template <class T> struct block_allocated_hash_set_node
{
  void *p1,*p2,*p3;
  T     t;
  int   i;
...
template <class Key,size_t chunk_size,class Tred = stdext::hash_compare<Key, _STD less<Key> > >
class block_allocated_hash_set: 
  public stdext::hash_set<Key,Tred,
                  block_allocator<Key,chunk_size,block_allocated_hash_set_node<Key> > >
{
  typedef typename 
    stdext::hash_set<
      Key,Tred,
  ...
  public:
    ...
    explicit block_allocated_hash_set(
      const Tred& comp=Tred(),
      const allocator_type& al=allocator_type()
    ):super(comp,al)
    {
    }

    block_allocated_hash_set(
      const value_type *first,const value_type *last,const Tred& comp=Tred(),
      const allocator_type& al=allocator_type()
    ):super(first,last,comp,al)
    {
    }
...
#endif
#endif /* DEFINE_BLOCK_ALLOCATED_HASH_SET */


#ifdef DEFINE_BLOCK_ALLOCATED_HASH_MAP
#ifndef BLOCK_ALLOCATED_HASH_MAP_DEFINED
#define BLOCK_ALLOCATED_HASH_MAP_DEFINED
...
template <class Key,class T> struct block_allocated_hash_map_node
{
  void            *p1,*p2,*p3;
  std::pair<Key,T> t;
  int              i;
...
template<class Key,class T,size_t chunk_size,class Tred = stdext::hash_compare<Key, _STD less<Key> > >
class block_allocated_hash_map:
  public stdext::hash_map<Key,T,Tred,
                  block_allocator<T,chunk_size,block_allocated_hash_map_node<Key,T> > >
{
  typedef typename 
    stdext::hash_map<
      Key,T,Tred,
  ...
  public:
    ...
    explicit block_allocated_hash_map(
      const Tred& comp=Tred(),
      const allocator_type& al=allocator_type()
    ):super(comp,al)
    {
...
#endif
#endif /* DEFINE_BLOCK_ALLOCATED_HASH_MAP */