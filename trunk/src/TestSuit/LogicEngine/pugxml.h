...
public:
	...
	//<summary>Cast attribute value as float. If not found, return 0.0.</summary>
	//<returns>Attribute value as float, or 0.0.</returns>
	//<remarks>Note: Modifying this will not change the value, e.g. read only.</remarks>
	operator float()
	{
...
public:
	...
	std::string pathaddress(const TCHAR* delimiter = _T("/"))
	{
		xml_node cursor = *this; //Make a copy.
        char szTemp[ 50 ];
		std::string spath( _itoa( (int)(xml_node_struct*)cursor, szTemp, 16 ) );

		while(cursor.moveto_parent() && !cursor.type_document()) //Loop to parent (stopping on actual root because it has no name).
		{
            _itoa( (int)(xml_node_struct*)cursor, szTemp, 16 );
...
public:
	...
	void clear()
	{
		...
		//if(_buffer){ free(_buffer); _buffer = 0; }commented by qiuxinyu _buffer from outside
#ifdef PUGOPT_MEMFIL
		close_memfile();
#endif
	}
